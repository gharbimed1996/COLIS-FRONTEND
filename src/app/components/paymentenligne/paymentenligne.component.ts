...
export class PaymentenligneComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder, private http: HttpClient) {
    this.paymentForm = this.formBuilder.group({
      ...
      intent: ['AUTHORIZE', Validators.required],
  ...
  onSubmit() {
    if (this.paymentForm.valid) {
      ...
      const apiUrl = "http://localhost:8090/createPayment"; // Remplacez par l'URL réelle de votre API pour les paiements Acme
      const headers = new HttpHeaders({
        'Content-Security-Policy': "default-src 'self' http://localhost:8090" // Remplacez http://localhost:9090 par l'URL de votre API
      });
